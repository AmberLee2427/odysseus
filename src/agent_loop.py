...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_latex_projects": "- ```manage_latex_projects``` — Read and manage local LaTeX/Overleaf project metadata, Git credential STATUS, authenticated Overleaf pulls, and explicit commit+push back to Overleaf. Args (JSON): {\"action\":\"credential_status|list|create|metadata_read|metadata_patch|pull|push|tree|status\", \"latex_project_id\":\"...\", \"message\":\"...\", ...}. Use this for Overleaf Git credential checks, project registration, status, tree, pull/clone, and user-confirmed push. `credential_status` returns only configured ids/usernames and NEVER returns tokens. Use `pull`, not bash/git clone, to fetch from Overleaf; use `push`/`commit_and_push`, not bash/git push, to sync confirmed local changes back. Do NOT use bash/grep/read_file to look for Overleaf credentials or worktree paths.",
