...
- Durable project notes, linked research notes, wiki/knowledge pages, tags, or backlinks → use `manage_logseq`. Do NOT use it for reminders/todos, and never tidy/delete Logseq pages unless the user explicitly asks.
...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_logseq": "- ```manage_logseq``` — Manage the shared Logseq knowledge graph. Args (JSON): {\"action\":\"status|list|search|read|write|append|backlinks\", \"title\":\"...\", \"content\":\"...\", \"properties\":{}, \"query\":\"...\", \"tag\":\"...\"}. Use for durable project/research notes, linked knowledge pages, tags, and backlinks. Do not use for reminders/todos or compact personal memory. Never tidy/delete graph pages unless explicitly requested.",
    ...
    "manage_latex_projects": "- ```manage_latex_projects``` — Read and manage local LaTeX/Overleaf project metadata, text files, Git credential STATUS, authenticated Overleaf pulls, and explicit commit+push back to Overleaf. Args (JSON): {\"action\":\"credential_status|list|create|metadata_read|metadata_patch|tree|read_file|replace_text|write_file|diff|pull|push|status\", \"latex_project_id\":\"...\", \"path\":\"bibliography.bib\", \"old_text\":\"...\", \"new_text\":\"...\", \"message\":\"...\"}. Use this for Overleaf Git credential checks, project registration, status, tree, reading/editing manuscript files by relative path, pull/clone, diff, and user-confirmed push. `credential_status` returns only configured ids/usernames and NEVER returns tokens. Use `read_file`/`replace_text`/`write_file`, not shell/grep/read_file, for LaTeX project files. Use `pull`, not bash/git clone; use `push`/`commit_and_push`, not bash/git push. Do NOT use bash/grep/read_file to look for Overleaf credentials or worktree paths.",
...
- Logseq knowledge: use the named `manage_logseq` tool instead of raw `/api/logseq/*` calls.
...
**When to prefer named tools over app_api:** if a named wrapper exists (list_email_accounts, list_emails, read_email, manage_calendar, manage_notes, manage_logseq, list_served_models, etc.) USE IT — it has nicer output formatting and clearer schema. Reach for `app_api` only when there's no wrapper for what you need.
...
_ADMIN_TOOLS = {
    "manage_session", "manage_skills", "manage_tasks",
    "manage_endpoints", "manage_mcp", "manage_webhooks", "manage_tokens",
    "manage_documents", "manage_logseq", "manage_settings", "create_session", "list_sessions",
