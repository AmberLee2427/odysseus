...
ASSISTANT_ALWAYS_AVAILABLE = frozenset({
    ...
    "manage_memory", "manage_logseq", "web_search", "read_file",
...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "read_file": "Read a file from disk and return its contents. View source code, config files, logs. Supports an optional line range for large files: offset is a 1-based line number and limit is a number of lines, not bytes.",
    ...
    "manage_logseq": "Manage the shared Logseq knowledge graph: search, read, write, append, inspect tags and backlinks. Use for durable project notes, linked knowledge pages, and research notes. Not for reminders/todos or compact personal memory.",
    ...
    "manage_latex_projects": "Read and manage local LaTeX/Overleaf project metadata, Git credential STATUS, project tree, git status, and authenticated Overleaf pulls/clones. Use for Overleaf Git credential checks, LaTeX project registration, metadata, pull from Overleaf, project tree, and git status. credential_status returns only configured ids and usernames, never tokens. Prefer this over bash, git clone, grep, read_file, or app_api for Overleaf/LaTeX project state.",
...
class ToolIndex:
    ...
    _KEYWORD_HINTS = {
        ...
        frozenset({"note", "todo", "reminder", "remind", "checklist", "remember to"}):
            {"manage_notes"},
        frozenset({"logseq", "knowledge graph", "knowledge base", "wiki", "backlink",
                   "backlinks", "linked note", "linked notes", "project notes",
                   "durable notes", "knowledge page", "knowledge pages"}):
            {"manage_logseq"},
        frozenset({"overleaf", "latex", "tex project", "manuscript", "git credential",
                   "git credentials", "credential configured", "credential status",
                   "latex project", "project tree", "main.tex", "pull overleaf",
                   "clone overleaf", "fetch overleaf"}):
