...
async def do_get_project_context(content: str) -> Dict:
    """Returns a payload of relevant file paths and metadata for a given project.
    Use this to 'activate' a project context.
    """
    from odysseus.services.knowledge_graph import KnowledgeGraphService
    import json
    try:
    ...
    if not project_name:
        return {"error": "project_name is required", "exit_code": 1}

    service = KnowledgeGraphService()
    context = service.get_project_context(project_name)
    if not context:
        return {"error": f"No project context found for '{project_name}'"}
    return {"action": "get_project_context", "project": project_name, "data": context}

async def do_get_context_by_tag(content: str) -> Dict:
    """Returns a list of file paths associated with a specific tag.
    Use this to pull in topical context (e.g., 'cosci').
    """
    from odysseus.services.knowledge_graph import KnowledgeGraphService
    import json
    try:
    ...
    tag = args.get("tag")
    if not tag:
        return {"error": "tag is required", "exit_code": 1}

    service = KnowledgeGraphService()
    context = service.get_context_by_tag(tag)
    if not context:
        return {"error": f"No context found for tag '{tag}'"}
    return {"action": "get_context_by_tag", "tag": tag, "data": context}
...
async def do_vault_unlock(content: str, owner: Optional[str] = None) -> Dict:
    """Unlock the Bitwarden vault and persist the returned session key."""
    try:
        args = _parse_tool_args(content)
    except ValueError:
        return {"error": "Invalid JSON arguments", "exit_code": 1}

    master_password = (args.get("master_password") or args.get("password") or "").strip()
    if not master_password:
        return {"error": "master_password is required", "exit_code": 1}

    stdout, stderr, rc = await _run_bw(
        ["unlock", "--raw"],
        input_text=master_password + "\n",
    )
    if rc != 0 or not stdout:
        return {"error": f"bw unlock failed: {stderr[:300] or 'no session returned'}", "exit_code": 1}

    from pathlib import Path
    cfg = _load_vault_config()
    cfg["session"] = stdout.strip()
    path = Path("data/vault.json")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(cfg, indent=2), encoding="utf-8")
    return {"output": "Vault unlocked. Session saved.", "exit_code": 0}


def do_get_project_context(project_name: str) -> Dict:
    """Return a list of relevant file paths and metadata for a given project name.
    Uses the project manifest generated by the graph scanner.
    """
    from odysseus.services.knowledge_graph import KnowledgeGraphService
    try:
        service = KnowledgeGraphService()
        context = service.get_project_context(project_name)
        if not context:
            return {"error": f"No context found for project: {project_name}"}
        return {"action": "get_project_context", "project": project_name, "context": context}
    except Exception as e:
        return {"error": f"Failed to get project context: {e}"}


def do_get_context_by_tag(tag: str) -> Dict:
    """Return a list of file paths associated with a specific tag.
    Useful for pulling in topical context.
    """
    from odysseus.services.knowledge_graph import KnowledgeGraphService
    try:
        service = KnowledgeGraphService()
        ...
        if not context:
            return {"error": f"No context found for tag: {tag}"}
        return {"action": "get_context_by_tag", "tag": tag, "context": context}
    except Exception as e:
        return {"error": f"Failed to get context by tag: {e}"}
